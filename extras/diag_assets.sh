...
# --- Phase 10: PU Command Hardening (sudo, unzip) ---
echo ""
echo "===== Testing: PU Command Hardening (sudo, unzip) ====="
delay 400
login root mcgoopis
echo "--- Test: visudo documentation ---"
man visudo | grep "RULE FORMAT"
echo "--- Test: unzip path traversal ---"
mkdir unzip_jail
cd unzip_jail
check_fail "unzip ../traversal.zip"
check_fail "ls ../pwned.txt"
cd ..
rm -r -f unzip_jail
echo "--- Test: unzip depth bomb ---"
check_fail "unzip ./depth_bomb.zip"
echo "--- Test: sudo timestamp invalidation ---"
useradd sudotester
sudopass
sudopass
echo "sudotester ALL" >> /etc/sudoers
logout
login sudotester
sudopass
run ../sudo_test_script.sh
logout
login sudotester
sudopass
echo "This next command should require a password prompt."
run ../sudo_reauth_test.sh
logout
login root mcgoopis
removeuser -f sudotester
# Cleanup sudoers file
visudo
# (This would be manual in a real test, here we simulate by recreating it)
echo "root    ALL" > /etc/sudoers
echo "%root   ALL" >> /etc/sudoers
# End manual cleanup
delay 700
logout
...
# --- Phase 11: Advanced Scripting (`run`) ---
echo ""
...
# --- Phase 12: Application Tests (paint, chidi) ---
echo ""
...
# --- Phase 13: Adventure Game Engine ---
echo ""
