...
touch new_file.tst; ls -l new_file.tst
delay 1600;
touch new_file.tst; ls -l new_file.tst
touch -d "1 year ago" old_file.tst; ls -l old_file.tst
ls -l
delay 700
echo "---------------------------------------------------------------------"
...
echo "Appended line" >> file_a.txt; cat file_a.txt
...
echo "smallest" > file_c.txt; echo "medium content here" > file_z.log
...
mkdir sub_ls; touch sub_ls/nested.sh; touch .hidden_adv.txt
delay 400
...
delay 400
login root mcgoopis
groupadd testgroup; useradd testuser; testpass; testpass; usermod -aG testgroup testuser
cd /home/userDiag/diag_workspace
echo "Initial content" > group_test_file.txt
chown userDiag group_test_file.txt; chgrp testgroup group_test_file.txt; chmod 664 group_test_file.txt
logout
...
logout
login root mcgoopis
removeuser -f testuser; groupdel testgroup
rm -f /home/userDiag/diag_workspace/group_test_file.txt
logout
login userDiag pantload; cd /home/userDiag/diag_workspace
delay 700
echo "---------------------------------------------------------------------"
...
run ./test_rm_no.sh; ls interactive_file.txt; run ./test_rm_yes.sh; check_fail "ls interactive_file.txt"
delay 300
echo "--- Test: 'cp -i' (interactive copy) ---"
run ./test_cp_no.sh; cat interactive_target.txt; run ./test_cp_yes.sh; cat interactive_target.txt
delay 300
echo "--- Test: 'mv -i' (interactive move) ---"
run ./test_mv_no.sh; ls mv_source_2.txt; cat mv_target_2.txt; run ./test_mv_yes.sh; cat mv_target_2.txt; check_fail "ls mv_source_2.txt"
delay 700
echo "---------------------------------------------------------------------"
...
alias l="ls"; alias la="ls -a"; alias; la; unalias l; check_fail "l"; unalias la
delay 300
echo "--- Test: Environment Variables (set, unset, expansion) ---"
set TEST_VAR "Hello OopisOS"; set; echo "Value is: $TEST_VAR"; unset TEST_VAR; echo "Value is now: $TEST_VAR"
delay 700
echo "---------------------------------------------------------------------"
...
delay 400
echo "--- Test: 'paint' file creation and content verification ---"
paint new_art.oopic
cat sample.oopic
check_fail "paint existing_dir"
echo "--- Test: 'chidi' command file/directory finding ---"
chidi ./chidi_test_docs
chidi ./chidi_test_docs/not_markdown.txt
delay 700
echo "---------------------------------------------------------------------"
...
delay 400
echo "--- Test 1: Launch and immediately quit custom game ---"; run ./adv_test1.sh
delay 300
echo "--- Test 2: Basic gameplay sequence ---"; run ./adv_test2.sh
delay 300
echo "--- Test 3: Saving game state ---"; run ./adv_test3.sh
echo "Verifying save file was created:"; ls my_save.sav
delay 300
echo "--- Test 4: Loading game state and verifying ---"; run ./adv_test4.sh
delay 700
