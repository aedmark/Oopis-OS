...
(() => {
    ...
    const lsCommandDefinition = {
        ...
        coreLogic: async (context) => {
            ...
            // *** FIX: Create a mutable set of flags that can be modified for non-interactive mode. ***
            ...
            async function listSinglePathContents(targetPathArg, currentFlags) {
                ...
                if (currentFlags.dirsOnly) {
                    const details = getItemDetails(targetPathArg, targetNode, pathValidation.resolvedPath);
                    if (details) singleItemResultOutput = currentFlags.long ? formatLongListItem(details) : details.name;
                    ...
                    for (const name of childrenNames) {
                        if (!currentFlags.all && name.startsWith(".")) continue;
                    ...
                    itemDetailsList = sortItems(itemDetailsList, currentFlags);
                } else {
                ...
                if (singleItemResultOutput !== null) {
                    ...
                    if (currentFlags.long) {
                        if (itemDetailsList.length > 0) currentPathOutputLines.push(`total ${itemDetailsList.length}`);
                        itemDetailsList.forEach(item => { currentPathOutputLines.push(formatLongListItem(item)); });
                    } else if (currentFlags.oneColumn) {
            ...
            let pathsToList = args.length > 0 ? args : ["."];
            let outputBlocks = [];
            ...
            if (effectiveFlags.recursive) {
                async function displayRecursive(currentPath, displayFlags, depth = 0) {
                    ...
                    if (listResult.items && FileSystemManager.getNodeByPath(currentPath)?.type === 'directory') {
                ...
                for (let i = 0; i < pathsToList.length; i++) {
                    const recursiveResult = await displayRecursive(pathsToList[i], effectiveFlags);
                ...
                for (const path of fileArgs) {
                    const listResult = await listSinglePathContents(path, effectiveFlags);
