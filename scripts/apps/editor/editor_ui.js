...
const EditorUtils = (() => {
    ...
    function lineColToIndex(lines, line, col) {
        let index = 0;
    ...
    return { determineMode, calculateStatusBarInfo, generateLineNumbersArray, lineColToIndex, escapeHTML };
})();
...
const EditorUI = (() => {
  ...
  `;
    ...
    function _calculateAndSetDimensions() {
        ...
        const tempLine = Utils.createElement('div', { className: 'editor-line', textContent: 'M' });
        ...
        charDimensions.width = rect.width > 0 ? rect.width : 8;
        charDimensions.height = lineHeight;

        elements.virtualContent.removeChild(tempLine);
    }

    function indexToLineCol(index) {
        let line = 0, col = 0, charCount = 0;
        for (let i = 0; i < lines.length; i++) {
            const lineLengthWithNewline = lines[i].length + 1;
            if (index < charCount + lineLengthWithNewline) {
                line = i;
                col = index - charCount;
                return { line, col };
            }
    ...
    function coordinatesToIndex(e) {
        if (!elements.virtualContent || !lineHeight || !charDimensions.width) return 0;
        const rect = elements.virtualContent.getBoundingClientRect();
        const scrollLeft = elements.virtualScroller.scrollLeft;
        ...
        const col = Math.max(0, Math.round(x / charDimensions.width));
    ...
    function buildLayout(callbacks) {
        ...
        mdButtonDetails.forEach(detail => {
            elements[detail.name] = Utils.createElement("button", { className: "editor-btn editor-btn--format", innerHTML: detail.iconHTML.replace(/#3eca02/g, 'currentColor'), title: detail.title, eventListeners: { click: eventCallbacks[detail.callbackName] } });
        ...
        htmlButtonDetails.forEach(detail => {
            elements[detail.name] = Utils.createElement("button", { className: "editor-btn editor-btn--format", textContent: detail.text, title: detail.title, eventListeners: { click: eventCallbacks[detail.callbackName] } });
        ...
        elements.selectionContainer = Utils.createElement('div', { className: 'editor-selection-container' });
        elements.cursorDiv = Utils.createElement("div", { className: 'editor-cursor' });
        elements.virtualContent = Utils.createElement("div", { id: "virtual-content", className: 'editor-line-container' }, elements.selectionContainer, elements.cursorDiv);
        elements.virtualScroller = Utils.createElement("div", { id: "virtual-scroller", className: 'editor-scroller' }, elements.virtualContent);
        elements.hiddenTextarea = Utils.createElement("textarea", { id: "hidden-textarea", className: 'editor-hidden-input', spellcheck: "false", autocapitalize: "none" });
        elements.contentContainer = Utils.createElement("div", { id: "editor-content-container", className: 'editor-content-wrapper' }, elements.virtualScroller, elements.hiddenTextarea);

        elements.virtualScroller.addEventListener('scroll', _handleScroll);
        elements.contentContainer.addEventListener('mousedown', (e) => {
            elements.hiddenTextarea.focus();
            const startIndex = coordinatesToIndex(e);
            eventCallbacks.onSelectionStart(startIndex);

            const onMouseMove = (moveEvent) => {
                const moveIndex = coordinatesToIndex(moveEvent);
                eventCallbacks.onSelectionDrag(moveIndex);
            };
    ...
    function _calculateVisibleLines() {
        ...
        const lineCount = Math.ceil(containerHeight / lineHeight) + 5;
    ...
    function _renderVisibleContent() {
        ...
        while (elements.virtualContent.firstChild && elements.virtualContent.firstChild.classList.contains('editor-line')) {
            elements.virtualContent.removeChild(elements.virtualContent.firstChild);
        }

        const fragment = document.createDocumentFragment();
        let lineStartIndex = EditorUtils.lineColToIndex(lines, visibleStartLine, 0);

        for (let i = visibleStartLine; i < visibleEndLine; i++) {
            const lineContent = lines[i];
            const lineElement = Utils.createElement("div", {
                className: 'editor-line',
                style: { position: 'absolute', top: `${i * lineHeight}px`, left: 0, width: '100%', height: `${lineHeight}px` }
            });


            const matchesOnLine = findMatches.filter(match => {
                const matchEnd = match.index + match[0].length;
                const lineEndIndex = lineStartIndex + lineContent.length;
                return match.index < lineEndIndex && matchEnd > lineStartIndex;
            });
            ...
            if (matchesOnLine.length > 0) {
                ...
                matchesOnLine.forEach(match => {
                    const relativeIndex = match.index - lineStartIndex;
                    const className = match.index === findActiveIndex ? 'highlight-match--active' : 'highlight-match';
                    html += EditorUtils.escapeHTML(lineContent.substring(lastIndex, relativeIndex));
                    html += `<span class="${className}">${EditorUtils.escapeHTML(match[0])}</span>`;
                    lastIndex = relativeIndex + match[0].length;
                });
                html += EditorUtils.escapeHTML(lineContent.substring(lastIndex));
            ...
            lineStartIndex += lineContent.length + 1;
        }

        elements.virtualContent.style.height = `${lines.length * lineHeight}px`;
        elements.virtualContent.prepend(fragment);
        _renderGutter();
    }
    ...
    function _renderGutter() {
        ...
        elements.lineGutter.style.transform = `translateY(-${elements.virtualScroller.scrollTop}px)`;
    }


    function updateCursor(start, end) {
        if (!elements.cursorDiv || !elements.selectionContainer || !lineHeight || !charDimensions.width) return;


        if (start > end) [start, end] = [end, start];

        const startPos = indexToLineCol(start);


        elements.cursorDiv.style.top = `${startPos.line * lineHeight}px`;
        elements.cursorDiv.style.left = `${startPos.col * charDimensions.width}px`;
        elements.cursorDiv.style.height = `${lineHeight}px`;


        while (elements.selectionContainer.firstChild) {
            elements.selectionContainer.removeChild(elements.selectionContainer.firstChild);
        }


        if (start !== end) {
            const endPos = indexToLineCol(end);
            const fragment = document.createDocumentFragment();

            for (let i = startPos.line; i <= endPos.line; i++) {
                const selDiv = Utils.createElement('div', { className: 'editor-selection' });
                const top = i * lineHeight;
                let left = 0;
                let width = 0;

                if (i === startPos.line) {
                    left = startPos.col * charDimensions.width;
                    width = (i === endPos.line)
                        ? (endPos.col - startPos.col) * charDimensions.width
                        : (lines[i].length - startPos.col) * charDimensions.width;
                } else if (i === endPos.line) {
                    left = 0;
                    width = endPos.col * charDimensions.width;
                } else {
                    left = 0;
                    width = lines[i].length * charDimensions.width;
                }

                selDiv.style.cssText = `top: ${top}px; left: ${left}px; width: ${width}px; height: ${lineHeight}px; display: block;`;
                fragment.appendChild(selDiv);
            }
            elements.selectionContainer.appendChild(fragment);
        }
    }
    ...
    function applyTextareaWordWrap(isWordWrapActive) {
        ...
        elements.virtualContent.style.wordBreak = isWordWrapActive ? 'break-all' : 'normal';
    }
        ...
        previewDebounceTimer = setTimeout(() => {
            if (isMarkdownMode) {
                if (typeof marked !== "undefined" && typeof DOMPurify !== "undefined") {
                    ...
                    elements.previewPane.innerHTML = DOMPurify.sanitize(dirty);
                } else {
                    elements.previewPane.textContent = "Markdown preview library (marked.js or DOMPurify.js) not loaded.";
                }
            } else if (isHtmlMode) {
                let iframe = elements.previewPane.querySelector("iframe");
                if (!iframe) {
                    iframe = Utils.createElement("iframe", { style: { width: '100%', height: '100%', border: 'none' }, sandbox: "" });
                    elements.previewPane.innerHTML = "";
    ...
    return {
        buildLayout, destroyLayout, updateFilenameDisplay, updateStatusBar, setTextareaContent, getTextareaContent,
        setEditorFocus, applyTextareaWordWrap, applyPreviewWordWrap, updateWordWrapButtonText, renderPreview,
        setViewMode, getPreviewPaneHTML, setGutterVisibility, elements, _updateFormattingToolbarVisibility,
        iframeStyles, updateFindBar, getFindQuery, getReplaceQuery, renderHighlights, initializeDimensions,
        updateCursor, indexToLineCol,
        getLines: () => lines
    };
})();